...
function cardHTML(m){
  ...
  // A finished game's pre-kickoff pick reads as confusing once the real
  // result is known and known to differ -- only show it for games still
  // to be decided.
  const pick=(op&&m.status!=='FINISHED')?`<div class="pick ${edge!=null&&Math.abs(edge)>=6?'edge':''} ${op.blocked?'gate':''}"><span class="pl">Pick</span><span class="pn">${esc(op.name)}</span><span class="pc">${esc(op.confidence??'—')}%</span><span class="pnote">${esc(op.note||'')}</span>${trend}</div>`:'';
