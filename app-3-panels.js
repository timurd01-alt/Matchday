...
h+=`<div class="status-grid" style="margin-top:10px">
	...
	<div class="statuscard info"><span class="slbl">Favorite vs. underdog ${metricHelp('Favorite vs. underdog','Favorite = the picked side had model probability 50% or higher. Underdog = a plurality pick below 50%, which only happens in three-way soccer markets.')}</span><div class="sval" style="font-size:.9rem;line-height:1.7">Favorite: <b>${pctf(sc.favorite_underdog?.favorite?.hits,sc.favorite_underdog?.favorite?.n)}</b> <i class="ssnote">(${sc.favorite_underdog?.favorite?.n||0})</i><br>Underdog: <b>${pctf(sc.favorite_underdog?.underdog?.hits,sc.favorite_underdog?.underdog?.n)}</b> <i class="ssnote">(${sc.favorite_underdog?.underdog?.n||0})</i></div><div class="hint">accuracy by model confidence &ge; 50%</div></div></div>`;
h+=`<div class="seclbl" style="margin-top:16px">Pick log</div>`+(sc.picks||[]).map(p=>{const done=!!p.result;const cls=done?(p.model_hit?'hit':'miss'):'wait';const badge=done?(p.model_hit?'HIT':'MISS'):'PENDING';return `<div class="scrow ${cls}"><span class="scst">${esc(p.stage||'')}</span><span class="scmatch">${esc(p.home)} v ${esc(p.away)}${p.score?` <b>${esc(p.score)}</b>`:''}</span><span class="scpick">${esc(p.pick_name||'')}${p.confidence?' · '+p.confidence+'%':''}${p.market_pick&&p.pick!==p.market_pick?' <i class="scsplit">vs market</i>':''}${p.value_side&&p.value_side!==p.pick?` <i class="scsplit vtag ${p.result?(p.value_hit?'vhit':'vmiss'):''}">chance: ${esc(p.value_name||'')} +${p.value_edge}${p.result?(p.value_hit?' &#10003;':' &#10007;'):''}</i>`:''}${scorecardUnderdogTag(p)}</span><span class="sctag ${cls}">${badge}</span>${p.legacy?`<span class="sctag legacy" title="${esc(p.integrity_label||'')}">LEGACY</span>`:''}</div>`}).join('');
...
if(next)parts.push(`<span class="ls-next ls-clickable" data-mid="${esc(next.id)}" onclick="openMatchModal(this.dataset.mid)" role="button" tabindex="0" title="Open expanded view">Next · <b>${esc(next.home.code)} v ${esc(next.away.code)}</b> ${kickIn(next.kickoff)}</span>`);else parts.push(`<span class="ls-next">No upcoming fixtures</span>`);parts.push(`<span class="ls-upd">${(()=>{try{const a=(Date.now()-new Date(DATA.updated))/60000;if(a>360)return `<b class="stale">data ${ago(DATA.updated)}</b> · `;}catch(e){}return 'Updated '+ago(DATA.updated)+' · ';})()}${t("independent · built for fans")} · <b style="color:var(--signal)">build ${currentBuild()}</b></span>`);$('#strip').innerHTML=parts.join('')}
...
function aggregateScorecards(datasets){
  ...
  out.favorite_underdog=combineSplit('favorite_underdog');
...
function renderStandings(){const comp=String(DATA.comp_key||'').toUpperCase();if(['MLB','NFL','NBA'].includes(comp)){const host=$('#view-groups'),st=deriveStandings();if(!st.length){host.innerHTML='<div class="vhead">Standings</div><div class="empty">No standings data found yet.</div>';return}_renderProStandings(st,host);return}renderGroups()}
...
function _v4FactorRows(pr,m){
  ...
  if(pr&&pr.data_availability){
    const availLabels={market:'market odds',box_score:'box score',lineups:'lineups',injuries:'injury reports'};
