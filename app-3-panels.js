...
async function load(manual=false){if(LOAD_TIMER){clearTimeout(LOAD_TIMER);LOAD_TIMER=null}try{
  ...
  if(!DATA_FILE&&!allSportsNeedsFull()){
    ...
    const summary=await fetch('board_summary.json?_='+Date.now()).then(r=>r.ok?r.json():null).catch(()=>null);
...
function fetchSportFile(key){
  ...
  if(!SPORT_FILE_CACHE[key])SPORT_FILE_CACHE[key]=fetch('data_'+key+'.json?_='+Date.now()).then(r=>r.ok?r.json():null).catch(()=>null);
...
function _v4MatchSnapshots(){
  ...
  return M.map(m=>{const pr=m.prediction||{},x=(m.markets||{})['1x2']||{};const probs=_v4ModelProbs(m);const twoWay=_isTwoWay(m);const hp=Math.round(Number(probs.h??x.home_pct??0)),dp=Math.round(Number(probs.d??x.draw_pct??0)),ap=Math.round(Number(probs.a??x.away_pct??0));const drawLine=twoWay?'':`<div class="probLine"><span class="sideName">Draw</span><span class="probTrack"><i class="probFill d" style="width:${Math.max(3,dp)}%"></i></span><span class="pct">${dp}%</span></div>`;return `<div class="matchSnapRow" onclick="openMatchModal('${esc(String(m.id||''))}')"><div><div class="matchSnapTeams">${esc(m.home?.code||m.home?.name||'H')} v ${esc(m.away?.code||m.away?.name||'A')}</div><div class="matchSnapMeta">${isMlbForecastPaused(m)?'Market odds · ':''}${esc(m.stage||'')} \u2013 ${kickIn(m.kickoff)}</div></div><div class="probLines"><div class="probLine"><span class="sideName">${esc(m.home?.code||'H')}</span><span class="probTrack"><i class="probFill h" style="width:${Math.max(3,hp)}%"></i></span><span class="pct">${hp}%</span></div>${drawLine}<div class="probLine"><span class="sideName">${esc(m.away?.code||'A')}</span><span class="probTrack"><i class="probFill a" style="width:${Math.max(3,ap)}%"></i></span><span class="pct">${ap}%</span></div></div></div>`}).join('');
}
