...
parts.push(isSample?`<span class="ls-badge sample">${t("sample data")}</span>`:live.length?`<span class="ls-badge live">${t("live data")}</span>`:`<span class="ls-badge ok">${t("live feed")}</span>`);if(live.length){const m=live[0];parts.push(`<span class="ls-tag"><span class="dot"></span>LIVE${m.minute?` ${m.minute}'`:''}</span>`);parts.push(`<span class="ls-live">${esc(m.home.code)} ${m.score?.home??0}–${m.score?.away??0} ${esc(m.away.code)}</span>`);if(live.length>1)parts.push(`<span class="ls-next">+${live.length-1} more live</span>`)}else parts.push(`<span class="ls-next">No live matches</span>`);if(next)parts.push(`<span class="ls-next">Next · <b>${esc(next.home.code)} v ${esc(next.away.code)}</b> ${kickIn(next.kickoff)}</span>`);else parts.push(`<span class="ls-next">No upcoming fixtures</span>`);parts.push(`<span class="ls-upd">${(()=>{try{const a=(Date.now()-new Date(DATA.updated))/60000;if(a>360)return `<b class="stale">data ${ago(DATA.updated)}</b> · `;}catch(e){}return 'Updated '+ago(DATA.updated)+' · ';})()}${t("analytics only, not betting advice")} · <b style="color:var(--signal)">build 0707B</b></span>`);$('#strip').innerHTML=parts.join('')}
