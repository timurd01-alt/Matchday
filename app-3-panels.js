...
<div class="statuscard info"><span class="slbl">Market favourite</span><div class="sval">${recf(sc.market_hits,sc.market_graded)}</div><div class="hint">${sc.market_graded?`${pctf(sc.market_hits,sc.market_graded)} correct — the benchmark`:'the benchmark — needs graded picks with market odds'}</div></div>
<div class="statuscard ${sc.disagree&&sc.disagree_hits/Math.max(1,sc.disagree)>=.5?'ok':'info'}"><span class="slbl">When we split with the market</span><div class="sval">${recf(sc.disagree_hits,sc.disagree)}</div><div class="hint">${sc.disagree?'model record on disagreements':'no graded picks that differed from the market yet'}</div></div>
...
h+=`<div class="status-grid" style="margin-top:10px">
	<div class="statuscard ${sc.clv_avg>0?'ok':'info'}"><span class="slbl">Closing line value ${metricHelp('Closing line value','Compares the model probability at lock with the final market snapshot.')}</span><div class="sval">${sc.clv_avg!=null?(sc.clv_avg>0?'+':'')+sc.clv_avg:'—'}</div><div class="hint">${sc.clv_n?`beat the close ${sc.clv_beat}/${sc.clv_n}${small} — the sharps' favourite metric`:'builds as picks grade'}</div></div>
	<div class="statuscard ${(sc.brier_advancement??sc.brier3)!=null&&(sc.brier_advancement??sc.brier3)<0.55?'ok':'info'}"><span class="slbl">${sc.brier_advancement!=null?'Advancement Brier':'3-way Brier'} ${metricHelp(sc.brier_advancement!=null?'Advancement Brier score':'Three-way Brier score',sc.brier_advancement!=null?'Grades the two-way probability of which team advances after extra time or penalties. Lower is better.':'Grades the full home, draw and away probability forecast. Lower is better.')}</span><div class="sval">${sc.brier_advancement??sc.brier3??sc.brier??'—'}</div><div class="hint">${sc.brier_advancement!=null?'grades who advances':'grades full home/draw/away probabilities'}${(sc.log_loss_advancement??sc.log_loss)!=null?` · log loss ${sc.log_loss_advancement??sc.log_loss}`:''}${small}</div></div>
	...
	<div class="statuscard info"><span class="slbl">Calibration ${metricHelp('Calibration','Checks whether events predicted at a given probability happen about that often.')}</span><div class="sval" style="font-size:.9rem;line-height:1.7">${(sc.calibration||[]).map(c=>`${c.band}%: <b>${Math.round(c.hits/c.n*100)}%</b> <i class="ssnote">(${c.n})</i>`).join('<br>')||'—'}</div><div class="hint">when we say X%, how often it happens</div></div></div>`;
h+=`<div class="seclbl" style="margin-top:16px">Pick log</div>`+(sc.picks||[]).map(p=>{const done=!!p.result;const cls=done?(p.model_hit?'hit':'miss'):'wait';const badge=done?(p.model_hit?'HIT':'MISS'):'PENDING';return `<div class="scrow ${cls}"><span class="scst">${esc(p.stage||'')}</span><span class="scmatch">${esc(p.home)} v ${esc(p.away)}${p.score?` <b>${esc(p.score)}</b>`:''}</span><span class="scpick">${esc(p.pick_name||'')}${p.confidence?' · '+p.confidence+'%':''}${p.market_pick&&p.pick!==p.market_pick?' <i class="scsplit">vs market</i>':''}${p.value_side&&p.value_side!==p.pick?` <i class="scsplit vtag ${p.result?(p.value_hit?'vhit':'vmiss'):''}">chance: ${esc(p.value_name||'')} +${p.value_edge}${p.result?(p.value_hit?' &#10003;':' &#10007;'):''}</i>`:''}${p.upset_score?` <i class="scsplit upsetTag">upset ${esc(p.upset_name||'')} ${esc(p.upset_score)}/100${p.upset_triggered?' active':''}${p.result?(p.upset_hit?' &#10003;':' &#10007;'):''}</i>`:''}</span><span class="sctag ${cls}">${badge}</span>${p.legacy?`<span class="sctag legacy" title="${esc(p.integrity_label||'')}">LEGACY</span>`:''}</div>`}).join('');
h+=`<div class="edisc">Verified pregame picks are the official record. Late or unverifiable legacy picks are excluded from accuracy. In knockout games, model picks are graded by who advances after extra time or penalties. Displayed scores remain factual and unchanged; market comparisons follow each market's settlement rules. Model output, not betting advice.</div>`;
host.innerHTML=h}
...
function renderCurrent(){captureSignalsIfFresh();({matches:renderMatches,results:renderResults,tott:renderTOTT,groups:renderGroups,title:renderTitle,advanced:renderAdvanced,edge:renderEdge,score:renderScore,bracket:renderBracket,third:renderThird,news:renderNews,insights:renderInsights,status:renderStatus,updates:renderSystemUpdates,community:renderCommunity,sandbox:renderSandbox,customize:renderCustomize}[VIEW]||renderMatches)();renderWelcome();highlightFavoriteRows();applyStaticI18n()}
...
function aggregateScorecards(datasets){
  ...
  const weighted=(key,countKey='graded',digits=3)=>{let total=0,n=0;sources.forEach(({sc})=>{const v=Number(sc[key]),w=Number(sc[countKey]??sc.graded);if(Number.isFinite(v)&&w>0){total+=v*w;n+=w}});return n?Number((total/n).toFixed(digits)):null};
  const out={graded:sum('graded'),pending:sum('pending'),model_hits:sum('model_hits'),
    market_graded:sum('market_graded'),market_hits:sum('market_hits'),disagree:sum('disagree'),disagree_hits:sum('disagree_hits')};
  out.brier=weighted('brier');out.brier3=weighted('brier3');out.log_loss=weighted('log_loss');
  out.advancement_graded=sum('advancement_graded');out.brier_advancement=weighted('brier_advancement','advancement_graded');out.log_loss_advancement=weighted('log_loss_advancement','advancement_graded');
...
function _modelWhen(m){if(m.status==='LIVE')return 'Result pending';if(m.status==='FINISHED')return 'Finished';if(isStaleUpcoming(m))return 'Past kickoff';return kickIn(m.kickoff)}
...
function _v4MatchSnapshots(){
  ...
  return M.map(m=>{const pr=m.prediction||{},x=(m.markets||{})['1x2']||{};const probs=_v4ModelProbs(m);const twoWay=_isTwoWay(m);const hp=Math.round(Number(probs.h??x.home_pct??0)),dp=Math.round(Number(probs.d??x.draw_pct??0)),ap=Math.round(Number(probs.a??x.away_pct??0));const drawLine=twoWay?'':`<div class="probLine"><span class="sideName">Draw</span><span class="probTrack"><i class="probFill d" style="width:${Math.max(3,dp)}%"></i></span><span class="pct">${dp}%</span></div>`;return `<div class="matchSnapRow" onclick="openMatchModal('${esc(String(m.id||''))}')"><div><div class="matchSnapTeams">${esc(m.home?.code||m.home?.name||'H')} v ${esc(m.away?.code||m.away?.name||'A')}</div><div class="matchSnapMeta">${esc(m.stage||'')} \u2013 ${kickIn(m.kickoff)}</div></div><div class="probLines"><div class="probLine"><span class="sideName">${esc(m.home?.code||'H')}</span><span class="probTrack"><i class="probFill h" style="width:${Math.max(3,hp)}%"></i></span><span class="pct">${hp}%</span></div>${drawLine}<div class="probLine"><span class="sideName">${esc(m.away?.code||'A')}</span><span class="probTrack"><i class="probFill a" style="width:${Math.max(3,ap)}%"></i></span><span class="pct">${ap}%</span></div></div></div>`}).join('');
}
...
function renderTitle(){
  ...
  let html=`<div class="forecastShell"><div class="marketBanner"><b>Still calibrating.</b> A lot of Matchday is still actively being built, and the model currently runs on free-tier data access -- so some predictions won\u2019t be as sharp as they could be yet. Treat these forecasts as a testing ground, not a finished read. As we tune what we have and can justify upgrading to better data sources, accuracy should keep improving -- real feedback from people using this is what shapes which upgrades happen next.</div><div class="forecastHero"><div><h2>Forecast board</h2><p>A cleaner view for tournament probabilities, upset risk, advancement paths, and scorer races. It avoids cramped rows and keeps the board focused on sports analytics rather than betting prompts.</p></div><div class="forecastKpis"><div class="forecastKpi"><span>Upcoming</span><b>${upcoming}</b></div><div class="forecastKpi"><span>Upset watch</span><b>${upsets.filter(x=>x.risk>=50).length}</b></div><div class="forecastKpi"><span>Title teams</span><b>${t.length||'—'}</b></div></div></div>`;
