...
if(next)parts.push(`<span class="ls-next ls-clickable" data-mid="${esc(next.id)}" onclick="openMatchModal(this.dataset.mid)" role="button" tabindex="0" title="Open expanded view">Next · <b>${esc(next.home.code)} v ${esc(next.away.code)}</b> ${kickIn(next.kickoff)}</span>`);else parts.push(`<span class="ls-next">No upcoming fixtures</span>`);parts.push(`<span class="ls-upd">${fallback&&syncedAt?`Predictions synced ${ago(syncedAt)}<span class="ls-extra"> · </span>`:fallback?`<b class="stale">data source ${ago(freshness.last_successful_at||DATA.updated)}</b><span class="ls-extra"> · </span>`:(()=>{try{const a=(Date.now()-new Date(DATA.updated))/60000;if(a>360)return `<b class="stale">data ${ago(DATA.updated)}</b><span class="ls-extra"> · </span>`;}catch(e){}return 'Updated '+ago(DATA.updated)+'<span class="ls-extra"> · </span>';})()}<span class="ls-extra">${t("independent · built for fans")} · <b style="color:var(--signal)">build ${currentBuild()}</b></span></span>`);$('#strip').innerHTML=parts.join('')}
