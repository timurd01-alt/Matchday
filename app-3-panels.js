...
<div class="statuscard info"><span class="slbl">Model record <button class="miniReload" title="Reload latest results" onclick="load(true)">\u21bb</button></span><div class="sval">${sc.model_hits}/${sc.graded}</div><div class="hint">${pctf(sc.model_hits,sc.graded)} of graded picks correct \u2014 verified pregame-locked picks only</div></div>${sc.combined&&sc.combined.graded?`<div class="statuscard warn" title="${esc(sc.combined.note||'')}"><span class="slbl">All-time total ${metricHelp('All-time total','Verified pregame-locked picks plus legacy picks that graded correctly but have no recoverable proof of when the pick was actually made. Shown separately from Model record so the verified number is never padded by unverifiable picks \u2014 see the pick log below for which is which.')}</span><div class="sval">${sc.combined.model_hits}/${sc.combined.graded}</div><div class="hint">${pctf(sc.combined.model_hits,sc.combined.graded)} incl. legacy \u00b7 verified ${sc.combined.verified_hits}/${sc.combined.verified_graded} \u00b7 legacy ${sc.combined.legacy_hits}/${sc.combined.legacy_graded}</div></div>`:''}
...
function _modelWhen(m){if(m.status==='LIVE'){const lc=liveClock(m);return lc?`LIVE ${lc}`:'LIVE'}if(m.status==='FINISHED')return 'Finished';if(isStaleUpcoming(m))return 'Past kickoff';return kickIn(m.kickoff)}
