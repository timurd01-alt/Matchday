...
if(isSample)parts.push(`<span class="ls-badge sample">${t("sample data")}</span>`);
...
if(next)parts.push(`<span class="ls-next ls-clickable" data-mid="${esc(next.id)}" onclick="openMatchModal(this.dataset.mid)" role="button" tabindex="0" title="Open expanded view">Next · <b>${esc(next.home.code)} v ${esc(next.away.code)}</b> ${kickIn(next.kickoff)}</span>`);else parts.push(`<span class="ls-next">No upcoming fixtures</span>`);parts.push(`<span class="ls-upd">${fallback&&syncedAt?`Predictions synced ${ago(syncedAt)}<span class="ls-extra"> · </span>`:fallback?`<b class="stale">data source ${ago(freshness.last_successful_at||DATA.updated)}</b><span class="ls-extra"> · </span>`:(()=>{try{const a=(Date.now()-new Date(DATA.updated))/60000;if(a>360)return `<b class="stale">data ${ago(DATA.updated)}</b><span class="ls-extra"> · </span>`;}catch(e){}return 'Updated '+ago(DATA.updated)+'<span class="ls-extra"> · </span>';})()}<span class="ls-extra">${t("Scorecard & Research update Sundays")} · ${t("independent · built for fans")} · <b style="color:var(--signal)">build ${currentBuild()}</b></span></span>`);$('#strip').innerHTML=parts.join('')}
...
function renderGroups(){const _tables=deriveStandings(),polls=_tables.filter(isPollTable),st=_tables.filter(g=>!isPollTable(g)),host=$('#view-groups'),sc=DATA.scorers||[];if(!st.length&&!polls.length){host.innerHTML=`<div class="vhead">${['NCAAF','NCAAM'].includes(DATA.comp_key)?'Conferences':'Groups'}</div><div class="empty">No group data found yet.</div>`;return}if(navProfile()==='soccer_league'){_v15RenderLeagueTable(st,host);return}const groupsTwoWay=SANDBOX_TWO_WAY.has(String(DATA.comp_key||'').toLowerCase());const americanSport=['us_sport','college','college_basketball'].includes(navProfile());const ratingSorted=false;const US_SCORE_UNIT={ncaaf:['PF','PA'],ncaam:['PF','PA']};const[fLabel,aLabel]=US_SCORE_UNIT[String(DATA.comp_key||'').toLowerCase()]||['GF','GA'];const winPct=t=>t.pld?((Number(t.w)||0)/t.pld*100).toFixed(1)+'%':'—';host.innerHTML=pollSectionHTML(polls)+`<div class="vhead">${['NCAAF','NCAAM'].includes(DATA.comp_key)?'Conferences':americanSport?'Standings':'Groups'}</div>`+st.map(g=>`<div class="tablewrap"><div class="groupHead">${americanSport?esc(g.group||'Full table'):esc(g.group)}<span>${['NCAAF','NCAAM'].includes(DATA.comp_key)?'Conference record, then head-to-head':americanSport?(ratingSorted?'ranked by model rating':'ranked by win rate'):'Top 2 · 3rd'}</span></div><table class="gtable"><thead><tr><th>Team</th>${americanSport?'<th title="0–10 blend of talent, Elo and season results">Power</th>':''}${['NCAAF','NCAAM'].includes(DATA.comp_key)?'<th title="Conference record: games against teams in this table only">Conf</th><th title="Non-conference record: every other game">Non-con</th><th title="Overall record">Overall</th>':''}<th>P</th><th>W</th>${groupsTwoWay?'':'<th>D</th>'}<th>L</th><th>${fLabel}</th><th>${aLabel}</th><th>${americanSport?'Diff':'GD'}</th><th>${americanSport?'Win%':'Pts'}</th><th>Form</th></tr></thead><tbody>${(g.teams||[]).map(t=>{const fl=uiFlag(t.code);const q=t.qual?`<span class="qbadge ${esc(t.qual.status)}" title="${esc(t.qual.note)}">${esc(t.qual.note)}</span>`:'';return `<tr class="${americanSport?'':(t.pos<=2?'qual':t.pos===3?'third':'')}"><td><div class="gteam teamClickable" data-team="${esc(t.name||'')}" onclick="openTeamModal(this.dataset.team)"><span class="pos">${t.pos||''}</span>${fl?`<span class="flagIcon">${fl}</span>`:''}<span class="code">${esc(t.code||'')}</span>${esc(t.name)} ${t.live?'<span class="liveMark">*</span>':''}${q}</div></td>${americanSport?`<td>${t.rating!=null?Number(t.rating).toFixed(2):'—'}</td>`:''}${['NCAAF','NCAAM'].includes(DATA.comp_key)?`<td><b>${esc(t.conf_record||'0-0')}</b></td><td>${Math.max(0,(Number(t.w)||0)-(t.cw||0))}-${Math.max(0,(Number(t.l)||0)-(t.cl||0))}</td><td>${Number(t.w)||0}-${Number(t.l)||0}</td>`:''}<td>${t.pld??'—'}</td><td>${t.w??'—'}</td>${groupsTwoWay?'':`<td>${t.d??'—'}</td>`}<td>${t.l??'—'}</td><td>${t.gf??'—'}</td><td>${t.ga??'—'}</td><td>${t.gd??'—'}</td><td><b>${americanSport?winPct(t):(t.pts??'—')}</b></td><td class="form">${esc(t.form||'')}</td></tr>`}).join('')}</tbody></table></div>`).join('')}
