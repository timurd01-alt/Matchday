...
h+=`<div class="status-grid" style="margin-top:10px">
	...
	<div class="statuscard ${sc.brier3!=null&&sc.brier3<0.55?'ok':'info'}"><span class="slbl">3-way Brier ${metricHelp('Three-way Brier score','Grades the full home, draw and away probability forecast. Lower is better.')}</span><div class="sval">${sc.brier3??sc.brier??'—'}</div><div class="hint">grades full home/draw/away probabilities${sc.log_loss!=null?` · log loss ${sc.log_loss}`:''}${small}</div></div>
...
function computeTeamProfile(name){
  ...
  return {
    ...
    pts: rec.pts??side?.pts??0, win_pct: rec.win_pct??null, form: rec.form||side?.form||'',
  ...
  return `<div class="tpHead"><button class="modalClose" onclick="closeTeamModal()" aria-label="Close">×</button>
    ...
    <div class="tpBody">
      <div class="tpStatGrid">
        ${twoWay?`<div class="tpStat"><span class="tpStatLbl">Win %</span><b>${p.win_pct!=null?(Number(p.win_pct)*100).toFixed(1)+'%':'—'}</b></div>`:`<div class="tpStat"><span class="tpStatLbl">Points</span><b>${p.pts}</b></div>`}
        <div class="tpStat"><span class="tpStatLbl">${twoWay?'Point diff':'Goal/point diff'}</span><b>${p.gd>0?'+':''}${p.gd}</b></div>
...
/* dedup */
function _v15RenderLeagueTable(st,host){const seen=new Set(),teams=[];(st||[]).forEach(g=>(g.teams||[]).forEach(t=>{const key=String(t.name||t.code||'').toLowerCase();if(key&&!seen.has(key)){seen.add(key);teams.push(t)}}));teams.sort((a,b)=>(Number(a.pos)||999)-(Number(b.pos)||999)||(Number(b.pts)||0)-(Number(a.pts)||0)||(Number(b.gd)||0)-(Number(a.gd)||0));const stale=DATA.standings_stale?`<div class="banner"><b>Showing last season's final table.</b> The new season's fixtures aren't published by our data source yet — this updates automatically once they are.</div>`:'';host.innerHTML=`<div class="vhead">Table</div>${stale}<div class="tablewrap leagueTableWrap"><div class="groupHead">${esc(DATA.competition||'League table')}<span>${teams.length} clubs · full table</span></div><table class="gtable"><thead><tr><th>Team</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>GD</th><th>Pts</th><th>Form</th></tr></thead><tbody>${teams.map((t,i)=>{const q=t.qual?`<span class="qbadge ${esc(t.qual.status||'')}" title="${esc(t.qual.note||'')}">${esc(t.qual.status||t.qual.note||'')}</span>`:'';return `<tr><td><div class="gteam teamClickable" data-team="${esc(t.name||'')}" onclick="openTeamModal(this.dataset.team)"><span class="pos">${t.pos||i+1}</span><span class="code">${esc(t.code||'')}</span>${esc(t.name||'')} ${q}</div></td><td>${t.pld??'—'}</td><td>${t.w??'—'}</td><td>${t.d??'—'}</td><td>${t.l??'—'}</td><td>${t.gf??'—'}</td><td>${t.ga??'—'}</td><td>${t.gd??'—'}</td><td><b>${t.pts??'—'}</b></td><td class="form">${esc(t.form||'')}</td></tr>`}).join('')}</tbody></table></div>`}
function _usSportGroupsLabel(){const prof=navProfile();return prof==='college'?'Top 25':prof==='college_basketball'?'Conferences':'Standings'}
function _v16RenderTop25(g){return `<div class="tablewrap top25Wrap"><div class="groupHead">${esc(g.group)}<span>national poll</span></div><table class="gtable top25Table"><thead><tr><th>Rank</th><th>Team</th><th>Record</th></tr></thead><tbody>${(g.teams||[]).map(t=>{const q=t.qual?`<span class="qbadge ${esc(t.qual.status||'')}" title="${esc(t.qual.note||'')}">${esc(t.qual.status||t.qual.note||'')}</span>`:'';return `<tr><td><span class="pos rankPos">${t.pos||''}</span></td><td><div class="gteam teamClickable" data-team="${esc(t.name||'')}" onclick="openTeamModal(this.dataset.team)"><span class="code">${esc(t.code||'')}</span>${esc(t.name||'')} ${q}</div></td><td><b>${esc(t.record||(t.w!=null?`${t.w??'—'}-${t.l??'—'}`:'—'))}</b></td></tr>`}).join('')}</tbody></table></div>`}
function _v16RenderRecordTable(g){return `<div class="tablewrap"><div class="groupHead">${esc(g.group)}<span>raw season records</span></div><table class="gtable ncaamTable"><thead><tr><th>Team</th><th>Record</th><th>Win%</th><th>PF/G</th><th>PA/G</th><th>Diff</th><th>Streak</th></tr></thead><tbody>${(g.teams||[]).map(t=>{const avgPf=t.avg_pf!=null?Number(t.avg_pf):(t.pld?Number(t.gf)/t.pld:null);const avgPa=t.avg_pa!=null?Number(t.avg_pa):(t.pld?Number(t.ga)/t.pld:null);return `<tr><td><div class="gteam teamClickable" data-team="${esc(t.name||'')}" onclick="openTeamModal(this.dataset.team)"><span class="pos">${t.pos||''}</span><span class="code">${esc(t.code||'')}</span>${esc(t.name||'')}</div></td><td><b>${esc(t.record||(t.w!=null?`${t.w??'—'}-${t.l??'—'}`:'—'))}</b></td><td>${t.win_pct!=null?(Number(t.win_pct)*100).toFixed(1)+'%':'—'}</td><td>${avgPf?avgPf.toFixed(1):'—'}</td><td>${avgPa?avgPa.toFixed(1):'—'}</td><td>${t.gd!=null?esc(t.gd):'—'}</td><td class="form">${esc(t.form||'—')}</td></tr>`}).join('')}</tbody></table></div>`}
function renderGroups(){const st=deriveStandings(),host=$('#view-groups'),prof=navProfile(),isUSFormat=prof==='college'||prof==='college_basketball'||prof==='us_sport';if(!st.length){host.innerHTML=`<div class="vhead">${isUSFormat?_usSportGroupsLabel():prof==='soccer_league'?'Table':'Groups'}</div><div class="empty">No group data found yet.</div>`;return}if(prof==='soccer_league'){_v15RenderLeagueTable(st,host);return}if(isUSFormat){host.innerHTML=`<div class="vhead">${_usSportGroupsLabel()}</div>`+st.map(g=>/top\s*25/i.test(g.group)?_v16RenderTop25(g):_v16RenderRecordTable(g)).join('');return}host.innerHTML=`<div class="vhead">Groups</div>${DATA.standings_stale?`<div class="banner"><b>Showing last season's final table.</b> The new season's fixtures aren't published by our data source yet — this updates automatically once they are.</div>`:''}`+st.map(g=>`<div class="tablewrap"><div class="groupHead">${esc(g.group)}<span>Top 2 · 3rd</span></div><table class="gtable"><thead><tr><th>Team</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>GD</th><th>Pts</th><th>Form</th></tr></thead><tbody>${(g.teams||[]).map(t=>{const fl=uiFlag(t.code);const q=t.qual?`<span class="qbadge ${esc(t.qual.status)}" title="${esc(t.qual.note)}">${esc(t.qual.note)}</span>`:'';return `<tr class="${t.pos<=2?'qual':t.pos===3?'third':''}"><td><div class="gteam teamClickable" data-team="${esc(t.name||'')}" onclick="openTeamModal(this.dataset.team)"><span class="pos">${t.pos||''}</span>${fl?`<span class="flagIcon">${fl}</span>`:''}<span class="code">${esc(t.code||'')}</span>${esc(t.name)} ${t.live?'<span class="liveMark">*</span>':''}${q}</div></td><td>${t.pld}</td><td>${t.w}</td><td>${t.d}</td><td>${t.l}</td><td>${t.gf}</td><td>${t.ga}</td><td>${t.gd}</td><td><b>${t.pts}</b></td><td class="form">${esc(t.form||'')}</td></tr>`}).join('')}</tbody></table></div>`).join('')}
/* dedup */
