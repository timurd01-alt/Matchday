...
<div class="statuscard info"><span class="slbl">Market at model lock</span><div class="sval">${recf(sc.market_hits,sc.market_graded)}</div><div class="hint">${sc.market_graded?`${pctf(sc.market_hits,sc.market_graded)} correct — identical fixtures and information cutoff`:sc.post_lock_market_graded?`${sc.post_lock_market_graded} later market snapshots excluded — not a fair same-time benchmark`:'needs a timestamped market quote captured by model lock'}</div></div>
<div class="statuscard ${sc.disagree&&sc.disagree_hits/Math.max(1,sc.disagree)>=.5?'ok':'info'}"><span class="slbl">Same-lock disagreements</span><div class="sval">${recf(sc.disagree_hits,sc.disagree)}</div><div class="hint">${sc.disagree?'model record where the locked model and same-time market differed':'no fairly timed disagreement sample yet'}</div></div>
