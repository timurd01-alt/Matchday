...
const NAV_DEF={
  all:         ['matches','results','advanced','community','sandbox','news','insights','status','updates','customize'],
  soccer_cup:  ['matches','results','groups','title','edge','advanced','score','bracket','third','tott','community','sandbox','news','insights','status','updates','customize'],
  soccer_club: ['matches','results','groups','title','edge','advanced','score','bracket','tott','community','sandbox','news','insights','status','updates','customize'],
  us_sport:    ['matches','results','groups','title','edge','advanced','score','community','sandbox','news','insights','status','updates','customize'],
  college:     ['matches','results','groups','bracket','title','edge','advanced','score','community','sandbox','news','insights','status','updates','customize'],
  college_basketball:['matches','results','groups','bracket','title','edge','advanced','score','community','sandbox','news','insights','status','updates','customize'],
  soccer_league:['matches','results','groups','title','edge','advanced','score','tott','community','sandbox','news','insights','status','updates','customize']
};
...
function renderSystemUpdates(){const host=$('#view-updates');const seen=localStorage.getItem('matchday.updates.lastSeen');const latest='build 0726A';host.innerHTML=`<div class="updatesShell"><div class="updatesHero"><section class="updatesIntro"><h2>System updates</h2><span class="safePill">UI</span></section><aside class="buildCard"><div class="tiny">Current build</div><div class="build">${esc(latest)}</div><div class="hint">Last viewed: ${seen?esc(ago(seen)):'not marked yet'}</div><div class="updateActions"><button class="miniBtn" onclick="markUpdatesRead()">Mark as read</button><button class="miniBtn" onclick="setView('status')">Open Status</button></div></aside></div><section class="timeline"><div class="timelineHead"><h3>Release notes</h3><span>${SYSTEM_UPDATES.length} entries</span></div>${SYSTEM_UPDATES.map(u=>`<article class="updateItem"><div class="updateDate">${esc(u.date)}</div><div><div class="updateTitle"><span>${esc(u.title)}</span><span class="updateBadge">${esc(u.tag)}</span></div><ul>${u.items.map(i=>`<li>${esc(i)}</li>`).join('')}</ul></div></article>`).join('')}</section></div>`}
