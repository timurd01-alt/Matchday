...
| Soccer event-data research | StatsBomb Open Data | Research candidate | Event-level shots, possessions, pressure, lineups, and xG for the competitions/seasons actually published | Intended for research and genuine football-analytics interest; published analysis requires StatsBomb credit/logo and acceptance of its user agreement. Coverage is selective, so it is best for offline method research, not assumed live coverage. Source: https://github.com/hudl/open-data |
| Domestic-soccer results history | OpenFootball `football.json` | Approved for pinned offline research | Completed EPL, Bundesliga, La Liga, Serie A, and Ligue 1 fixtures/results for deeper Elo/SRS research | CC0/public domain at reviewed revision `a5dd38b3bcbe3aa2477cf400f569264253d51431`. Pin the revision, preserve source-file provenance, validate identities/scores/overlap, and keep it research-only until prospective gates pass. It is not a live or personnel source. Source: https://github.com/openfootball/football.json |
| MLB historical game/event research | Retrosheet downloads | Approved for the implemented historical challenger | Historical play-by-play, game context, outcome rates, and independent feature validation | Official notice reviewed 2026-07-29. Retrosheet permits broad use but requires its specified attribution statement to appear prominently when data or a derived product is transferred. The frozen derived model is historical evidence; live features come only from the licensed feed. Source: https://www.retrosheet.org/game.htm |
