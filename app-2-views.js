...
function renderTOTT(){const host=$('#view-tott');const t=DATA.team_of_tournament;
  ...
  const line=(label,arr)=>`<div class="tottLine"><div class="tottLbl">${label}</div>${arr.length?`<div class="tottRow">${arr.map(p=>`<div class="tottCard${p.team_defense?' teamDefense':''}"><div class="tottName">${esc(p.name||'')}</div><div class="tottTeam">${esc(p.code||p.team||'')}</div><div class="tottStat">${esc(p.stat||`${p.goals}G ${p.assists}A`)}</div></div>`).join('')}</div>`:`<div class="tottEmpty">No qualifying player yet.</div>`}</div>`;
...
renderCustomize=function(){
  ...
  if(!grid.querySelector('.favoriteTeamSetting')){
  ...
  if(!grid.querySelector('.alertSettings')){
    ...
    alerts.innerHTML=`<label>Alert preferences</label><div class="alertSettingList"><label><span>Kickoff reminders<small>Watched and favorite teams</small></span><input type="checkbox" ${checked(SETTINGS.alertsKickoff)} onchange="updateSetting('alertsKickoff',this.checked)"></label><label><span>Live scores<small>Watched and favorite teams</small></span><input type="checkbox" ${checked(SETTINGS.alertsLive)} onchange="updateSetting('alertsLive',this.checked)"></label><label><span>Upset radar<small>Important live signals</small></span><input type="checkbox" ${checked(SETTINGS.alertsUpset)} onchange="updateSetting('alertsUpset',this.checked)"></label><label><span>Model movement<small>Probability and market gaps</small></span><input type="checkbox" ${checked(SETTINGS.alertsModel)} onchange="updateSetting('alertsModel',this.checked)"></label><label><span>Data health<small>Delayed or stale feeds</small></span><input type="checkbox" ${checked(SETTINGS.alertsData)} onchange="updateSetting('alertsData',this.checked)"></label></div><div class="hint">Alerts stay inside Matchday for now. Browser push can be added after accounts and consent controls exist.</div>`;
    grid.append(alerts);
  }
};
