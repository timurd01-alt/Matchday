...
(function(){
  ...
  function researchSignalsPanel(m){
    ...
    const weight='production weight 0';
    const caution='These fields expand the reasoning record. They do not change the official probability until a frozen out-of-sample and prospective promotion gate passes.';
    return `<section class="analystPanel researchPanel"><div class="researchHead"><div><span>Research signals</span><b>${esc(meta?.source||'Advanced profile')}</b></div><em>${esc(weight)}</em></div><div class="researchTeams"><b>${esc(m?.home?.code||m?.home?.name||'Home')}</b><span>authorized derived profile</span><b>${esc(m?.away?.code||m?.away?.name||'Away')}</b></div><div class="researchMetrics">${metrics}</div><div class="researchReceipt"><span>${esc(coverageText(meta)||'point-in-time shadow receipt')}</span><span>${esc(meta?.license||'research artifact')}</span></div><p class="researchCaution">${esc(caution)}</p></section>`;
  }
