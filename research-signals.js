...
(function(){
  ...
  function shadowBlock(m){
    ...
    return `<div class="researchShadow"><div><span>Official home probability</span><b>${official==null?'—':esc(`${official.toFixed(1)}%`)}</b></div><div><span>Calibrated Elo shadow</span><b>${calibrated==null?'—':esc(`${(calibrated*100).toFixed(1)}%`)}</b></div><div><span>Learned shadow</span><b>${learned==null?'—':esc(`${(learned*100).toFixed(1)}%`)}</b></div>${evidence}</div><p class="researchCaution">Calibrated Elo beat raw Elo in historical out-of-sample testing and is now collecting prospective evidence with zero probability weight. The learned NFL residual failed its promotion gate and remains audit-only.</p>`;
  }
